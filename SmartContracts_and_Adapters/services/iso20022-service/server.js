...
app.post("/iso/:tokenId/tsin006", (req, res) => {
  metrics.gen_in++;
  const tokenId = String(req.params.tokenId);
  const b = ensureBase(req.body);
  log("info", "tsin006:in", { tokenId, body: bodyPreview(req.body) });
  const xml = TSIN006(b);
  return res.json(save(tokenId, "tsin", "006", xml, b));
});

app.post("/iso/:tokenId/tsin007", (req, res) => {
  metrics.gen_in++;
  const tokenId = String(req.params.tokenId);
  const b = ensureBase(req.body);
  log("info", "tsin007:in", { tokenId, body: bodyPreview(req.body) });
  const xml = TSIN007(b);
  return res.json(save(tokenId, "tsin", "007", xml, b));
});

app.post("/iso/:tokenId/tsin008", (req, res) => {
  metrics.gen_in++;
  const tokenId = String(req.params.tokenId);
  const b = ensureBase(req.body);
  log("info", "tsin008:in", { tokenId, body: bodyPreview(req.body) });
  const xml = TSIN008(b);
  return res.json(save(tokenId, "tsin", "008", xml, b));
});

app.post("/iso/:tokenId/pacs008", (req, res) => {
  metrics.gen_in++;
  const tokenId = String(req.params.tokenId);
  const b = ensureBase(req.body);
  log("info", "pacs008:in", { tokenId, body: bodyPreview(req.body) });
  const xml = PACS008(b);
  return res.json(save(tokenId, "pacs", "008", xml, b));
});

app.post("/iso/:tokenId/camt054", (req, res) => {
  metrics.gen_in++;
  const tokenId = String(req.params.tokenId);
  const b = ensureBase(req.body);
  log("info", "camt054:in", { tokenId, body: bodyPreview(req.body) });
  const xml = CAMT054(b);
  return res.json(save(tokenId, "camt", "054", xml, b));
});
